...
const runTests = async () => {
    try {
        ...
        const adminLogin = await axios_1.default.post(`${API_URL}/login`, { email: 'admin@boletin360.com', password: 'password' });
        adminToken = adminLogin.data.token;
        console.log('Admin Login Success');
        console.log('\n--- 2. Create User (Admin) - Audit Log Check ---');
        const newUser = await axios_1.default.post(`${API_URL}/users`, {
            nombres: 'Test',
            apellidos: 'Teacher',
        ...
        const teacherLogin = await axios_1.default.post(`${API_URL}/login`, { email: 'testteacher@boletin360.com', password: 'password123' });
        ...
        await axios_1.default.post(`${API_URL}/users/${createdUserId}/reset-password`, {
            newPassword: 'newpassword123'
        }, { headers: { Authorization: `Bearer ${adminToken}` } });
        console.log('Password Reset Success');
        console.log('\n--- 7. Verify New Password Login ---');
        await axios_1.default.post(`${API_URL}/login`, { email: 'testteacher@boletin360.com', password: 'newpassword123' });
        console.log('Login with new password Success');
        console.log('\n--- 8. Cleanup (Delete User) ---');
        await axios_1.default.delete(`${API_URL}/users/${createdUserId}`, { headers: { Authorization: `Bearer ${adminToken}` } });
